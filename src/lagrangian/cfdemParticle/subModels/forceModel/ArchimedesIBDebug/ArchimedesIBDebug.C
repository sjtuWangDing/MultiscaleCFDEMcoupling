...
#include "ArchimedesIBDebug.H"
#include "addToRunTimeSelectionTable.H"
#include "voidFractionModel.H"

namespace Foam {

defineTypeNameAndDebug(ArchimedesIBDebug, 0);
...
ArchimedesIBDebug::ArchimedesIBDebug(const dictionary& dict, cfdemCloud& sm):
    forceModel(dict,sm),
    ...
    #endif
{
    ...
    particleCloud_.probeM().vectorFields_.append("archimedesIBDebugForce");  //first entry must the be the force
...
ArchimedesIBDebug::~ArchimedesIBDebug() {}

void ArchimedesIBDebug::setForce() const {
    vector force;
    ...
    for(int index = 0;index <  particleCloud_.numberOfParticles(); ++index)
    {
        ...
        for(int subCell=0;subCell<particleCloud_.cellsPerParticle()[index][0];subCell++)
        {
            ...
            if (cellI > -1) // particle Found
            {
                ...
                force = -g_.value() * forceSubM(0).rhoField()[cellI] * volume;
                break;
            }
        }

        Pout << "force: " << force[0] << ", " << force[1] << ", " << force[2] << endl;
        particleCloud_.Barrier(0.5);
        ...
        if(twoDimensional_) Warning<<"ArchimedesDebugIB model doesn't work for 2D right now!!\n"<< endl;
