...
void smoothingModel::smoothenAbsolutField(volScalarField& scalField) const {
  // (1) make the field volume-specific, equal to:
  // forAll(scalField, cellI) { scalField[cellI] /= mesh_.V()[cellI]; }
...
void smoothingModel::smoothenAbsolutField(volVectorField& vecField) const {
  // (1) make the field volume-specific, equal to:
  // forAll(vecField, cellI) { vecField[cellI] /= mesh_.V()[cellI]; }
