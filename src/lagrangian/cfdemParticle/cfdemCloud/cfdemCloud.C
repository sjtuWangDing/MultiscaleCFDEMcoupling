...
#if __MIXCLOUD__
double Foam::cfdemCloud::fineParticleRatio_ = 0.0;
double Foam::cfdemCloud::coarseParticleRatio_ = 0.0;
#endif  // __MIXCLOUD__
...
Foam::cfdemCloud::cfdemCloud(const fvMesh& mesh):
...
#if __MIXCLOUD__
  fineParticleRatio_ = couplingProperties_.lookupOrDefault<double>("fineParticleRatio", 3.0);
  ...
  // apply flag
  if (!debugMode()) {
    ddtVoidfraction_.writeOpt() = IOobject::NO_WRITE;
  }
  if (!debugMode()) {
    turbulenceMultiphase_.writeOpt() = IOobject::NO_WRITE;
  }
  ...
  if (couplingProperties_.found("ignore")) {
    ignore_ = true;
  }
  ...
  for (int i = 0; i < momCoupleModels_.size(); i++) {
    momCoupleModel_[i] = momCoupleModel::New(couplingProperties_, *this, momCoupleModels_[i]);
  ...
  for (int i = 0; i < nrForceModels(); i++) {
    forceModel_[i] = forceModel::New(couplingProperties_, *this, forceModels_[i]);
    forceModel_[i]().applyDebugSettings(debugMode());
  }
  if (nrForceModels() < SMALL) {
    FatalError << "Please use at least one forceModel ! (e.g. noDrag) \n"
      << abort(FatalError);
  }
  ...
  for (int i = 0; i < liggghtsCommandModelList_.size(); i++) {
    liggghtsCommand_[i] = liggghtsCommandModel::New(liggghtsCommandDict_, *this, liggghtsCommandModelList_[i], i);
  }
  ...
  if (nPatchesCyclic > 0 && nPatchesNonCyclic > 0) {
    if (verbose_) {
      Info << "nPatchesNonCyclic = " << nPatchesNonCyclic << ", nPatchesCyclic = " << nPatchesCyclic << endl;
    }
  ...
  if (mesh_.time().controlDict().lookupOrDefault<Switch>("adjustTimeStep", false) && !allowAdjustTimeStep_) {
    FatalError << "cfdemCloud::cfdemCloud(): you want to adjustTimeStep in controlDict. This is not allowed in this version of CFDEM." << abort(FatalError);
  }
  ...
  if (mesh_.cells().size() < 1) {
    ...
    if (nprocs > 2) {
      FatalError << endl << "cfdemCloud::cfdemCloud(): local mesh has zero cells. Please check the mesh and the decomposition!" << abort(FatalError);
    }
    Pout << "WARNING: cfdemCloud::cfdemCloud(): local mesh has zero cells. Please check the mesh and the decomposition!" << endl;
  }
  Info << "\ncfdemCloud::cfdemCloud(const fvMesh&) - done\n" << endl;
}
...
Foam::cfdemCloud::~cfdemCloud() {
  ...
  for(std::vector<double**>::iterator it = particleDatFieldsUserCFDEMToExt.begin();
      it != particleDatFieldsUserCFDEMToExt.end(); ++it) {
...
void Foam::cfdemCloud::giveDEMdata() {
  // if (verbose_) { Info << "Foam::cfdemCloud::giveDEMdata()" << endl; }
...
void Foam::cfdemCloud::giveUSERdata() {
  if (verbose_) { Info << "Foam::cfdemCloud::giveUSERdata()" << endl; }

  // Handover USER-defined data
  for(std::vector<word>::iterator it = namesFieldsUserCFDEMToExt.begin();
      it != namesFieldsUserCFDEMToExt.end(); ++it) {
    int positionInRegister = std::distance(namesFieldsUserCFDEMToExt.begin(), it);
    dataExchangeM().giveData(namesFieldsUserCFDEMToExt[positionInRegister],
                             "scalar-atom",
...
void Foam::cfdemCloud::setVoidFraction() {
  cfdemCloud::voidFractionM().setvoidFraction(NULL, voidfractions_,
                                              particleWeights_,
                                              particleVolumes_,
                                              particleV_);
}
...
void Foam::cfdemCloud::setParticleForceField() {
  averagingM().setVectorSum(forceM(0).impParticleForces(),
                            impForces_, particleWeights_, NULL);
  averagingM().setVectorSum(forceM(0).expParticleForces(),
                            expForces_, particleWeights_, NULL);
}
...
bool Foam::cfdemCloud::evolve(volScalarField& alpha,
                              volVectorField& Us,
                              volVectorField& U) {
  numberOfParticlesChanged_ = false;
  arraysReallocated_ = false;
  ...
  if (!ignore()) {
    ...
    }  // end of dataExchangeM().couple()

    // CHECK JUST TIME-INTERPOATE ALREADY SMOOTHENED VOIDFRACTIONNEXT AND UsNEXT FIELD 
    // IMPLICIT FORCE CONTRIBUTION AND SOLVER USE EXACTLY THE SAME AVERAGED QUANTITIES AT THE GRID!
    ...
    if (doCouple) {
      ...
      if (verbose_) { Info << "calcMultiphaseTurbulence()" << endl; }
...
// \brief 计算等效粘性系数
tmp<volScalarField> cfdemCloud::NuEff() const {
...
// \note - dev2(fvc::grad(U)().T()) 返回 grad(U) 转置的非对称分量 = fvc::grad(U)().T() - 2 / 3 * tr(fvc::grad(U)().T()) * I(单位二阶张量)
...
// tmp<fvVectorMatrix> cfdemCloud::ddtVoidfractionU(volVectorField& U,
//                                                  volScalarField& voidfraction) const {
...
void cfdemCloud::registerNamesFieldsUserCFDEMToExt(word fieldToRegister,
                                                   int& positionInRegister) {
...
bool cfdemCloud::checkAndregisterNamesFieldsUserCFDEMToExt(const wordList names,
                                                           std::vector<int> & positionInRegister) {
...
void cfdemCloud::allocateParticleDatFieldsUserCFDEMToExt() const {
  if (particleDatFieldsUserCFDEMToExt.size() > 0) {
    FatalError << "cfdemCloud::allocateParticleDatFieldsUserCFDEMToExt(): you are attempting to allocate fields in a container that already contains elements. This is not allowed, please clear container."
      << abort(FatalError);
  }
  //Go through list and allocate
  for(std::vector<word>::const_iterator it = namesFieldsUserCFDEMToExt.begin();
      it != namesFieldsUserCFDEMToExt.end(); ++it) {
...
void cfdemCloud::accessParticleDatFieldsUserCFDEMToExt(word fieldToAccess, double**& fieldData) {
  ...
  if (it != namesFieldsUserCFDEMToExt.end()) {
    ...
    FatalError << "field " << fieldToAccess << " not found." << abort(FatalError);
  }
}
