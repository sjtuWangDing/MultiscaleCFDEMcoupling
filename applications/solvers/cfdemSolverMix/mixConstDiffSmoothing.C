...
#include "mixConstDiffSmoothing.H"
#include "addToRunTimeSelectionTable.H"

namespace Foam {

defineTypeNameAndDebug(mixConstDiffSmoothing, 0);
...
mixConstDiffSmoothing::mixConstDiffSmoothing(const dictionary& dict, cfdemCloud& sm):
  ...
  smoothingLength_(dimensionedScalar("smoothingLength",
                                     dimensionSet(0, 1, 0, 0, 0, 0, 0),
                                     readScalar(propsDict_.lookup("smoothingLength")))),
  smoothingLengthReferenceField_(dimensionedScalar("smoothingLengthReferenceField",
                                                   dimensionSet(0, 1, 0, 0, 0, 0, 0),
                                                   readScalar(propsDict_.lookup("smoothingLength")))),
...
void mixConstDiffSmoothing::smoothen(volScalarField& fieldSrc) const {
  // Create scalar smooth field from oirgin scalar smooth field template
...
void mixConstDiffSmoothing::smoothen(volVectorField& fieldSrc) const {
  // Create vector smooth field from origin vector smooth field template
...
void mixConstDiffSmoothing::smoothenReferenceField(volVectorField& fieldSrc) const {
  // Create vector smooth field from origin vector smooth field template
  volVectorField vSmoothField = vSmoothField_;
  ...
  DT_.value() = smoothingLengthReferenceField_.value() * smoothingLengthReferenceField_.value() / deltaT.value();
  ...
  forAll(vSmoothField, cellI) {
    // have a vector in the OLD vSmoothField, so keep it!
    if (mag(vSmoothField.oldTime().internalField()[cellI]) > 0.0f) {
